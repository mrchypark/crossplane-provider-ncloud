...
import (
	...
	apisNamespaced "github.com/mrchypark/crossplane-provider-ncloud/apis/namespaced"
	"github.com/mrchypark/crossplane-provider-ncloud/config"
	"github.com/mrchypark/crossplane-provider-ncloud/internal/clients"
	controllerNamespaced "github.com/mrchypark/crossplane-provider-ncloud/internal/controller/namespaced"
	"github.com/mrchypark/crossplane-provider-ncloud/internal/features"
	"github.com/mrchypark/crossplane-provider-ncloud/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Ncloud").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("crossplane-provider-ncloud"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-crossplane-provider-ncloud",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Ncloud APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("crossplane-provider-ncloud:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Ncloud controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Ncloud controllers")
	}
